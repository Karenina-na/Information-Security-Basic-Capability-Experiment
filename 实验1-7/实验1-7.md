...
```blank
分析ssh登录日志文件，将登录失败超过20次(不含20)的IP加入到主目录的ban waitlist中，已有IP不重复加入
```

```bash
grep "Failed password for XXXXX from" ./auth.log | awk '{print $11}' | sort | uniq -c | awk '$1>20 {print $2}' >> ban_waitlist
```

```blank
从 ./auth.log 文件中搜索所有带有 "Failed password" 的行，然后使用 awk 命令提取每行的第11个单词，即IP地址。接着，使用 sort 命令对IP地址进行排序，再使用 uniq -c 命令统计每个IP地址出现的次数。然后，使用 sort -nr 命令按照IP地址出现的次数对结果进行排序。最后，使用 awk 命令只输出登录失败次数超过20次的IP地址列表。
```